...
**Data pipeline:**
- `supabase/functions/fetch-shorts` (Edge Function) runs on a daily `pg_cron` (04:00 UTC) — pulls YouTube Data API shorts per destination into the `shorts` table, using a whitelist filter (must show item/맛집 signal) rather than a blocklist.
...
- `.github/workflows/generate-static-pages.yml` runs `npm run generate` daily at 04:30 UTC (after the Supabase cron) and auto-commits/pushes if `travel/`, `watch/`, or `sitemap.xml` changed. Pull before pushing to this repo to avoid conflicting with this bot's commits.
