...
/* 로테이션·이력 길이·영상 선택 규칙은 scripts/card-rotation.js 에 있다(로테이션이 왜
   20곳이 아니라 10곳인지도 거기 적어뒀다). 큐레이션 리포트가 같은 규칙으로 앞으로
   2주 카드를 미리 계산하기 때문에 한 곳에 모았다. */
const { slugForDate, pushHistory } = require('./card-rotation.js');
...
function main() {
  const now = new Date();
  const { slug, dayOfYear, dateIso } = slugForDate(now);
  ...
  const nextHistory = pushHistory(history, { date: dateIso, slug, youtube_id: probe.youtube_id });
